...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE posts SET title = ?, post_type = ?, category = ?, content = ? WHERE post_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssi", $title, $post_type, $category, $content, $post_id);
    $stmt->execute();
    ...
    <div class="content flex-center">
        <div class="popup w-full">
          <form class="form" method="post">
            ...
            <button class="submit" style="width: 100%">Update Post</button>
          </form>
        </div>
    </div>
</body>
